...
// SetReuseFrames allows the Framer to reuse http2 Frame objects.
// If called on a Framer, Frame objects returned by calls to ReadFrame are only
// valid until the next call to ReadFrame. Note that Frame reusde is only
// currenlty implemented for DataFrames, but other type of Frames may be reused in
// the future.
